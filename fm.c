...
void setup_fm(int state) {
    ...
    mmap_clk = (unsigned char *)mmap(
                  ...
                  MAP_SHARED,
                  mem_fd,
                  0x3F000000  // See http://www.raspberrypi.org/forums/viewtopic.php?t=8476
              );
    ...
    //
    struct GPFSEL0_T { // all pins to alternate function 3
        char FSEL0 : 3;
        char FSEL1 : 3;
        char FSEL2 : 3;
        char FSEL3 : 3;
...
void playWav(char *filename, int mod, float bw) {
    ...
    lseek(fp, 44, SEEK_SET); //Skip header
