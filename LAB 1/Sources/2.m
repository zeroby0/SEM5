...
%%%%%%%%%
%minimum FFT size determined by desired freq Nmin=max(ceil(1/(df_desired*dt)),length(x));
%choose FFT size to be the next power of 2
Nfft = 2^(nextpow2(Nmin))
%compute Fourier transform, centering around X=dt*fftshift(fft(x,Nfft));
...
res or length of x
DC
%phase shift associated with start time X=X.*exp(-j*2*pi*f*tstart);
end
